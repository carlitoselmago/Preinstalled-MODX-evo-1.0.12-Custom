...
if (!is_file($fullfile = $modx->config['base_path'].$p_img)) return $nophoto;
...
umask(0777);
$mask = intval($modx->config['new_folder_permissions'], 8);
if (!is_dir($modx->config['base_path'].$mp_dir)) {
	if (!@mkdir($modx->config['base_path'].$mp_dir, $mask)) return $nophoto;
	chmod($modx->config['base_path'].$mp_dir, $mask);
}
...
chmod($modx->config['base_path'].$mp_img, intval($modx->config['new_file_permissions'], 8));
